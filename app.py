...
from db import init_db, create_project, upsert_tasks, fetch_project, fetch_tasks, latest_percent_by_task_id, save_progress_updates
...
- **Contingency**: reserve (~7–8%) for unforeseen conditions in renovations.
    ...
    if upload and st.button("Generate Baseline (Parse + Save)"):
        st.session_state.pid = parse_and_save(upload, project_name, start_date, hours_per_day, base_rate_inr, labour_burden, inefficiency, contingency)
    ...
    with col1:
        areas = sorted(df_tasks["Area"].dropna().unique().tolist())
        areas_sel = st.multiselect("Filter by Area", areas, default=areas[:5] if areas else [])
    with col2:
        trades = sorted(df_tasks["Trade"].dropna().unique().tolist())
    ...
    for _, r in df_tasks.iterrows():
        a = Activity(
            name=r["name"], area=r["Area"], trade=r["Trade"], description=r.get("Description",""),
    ...
    ev = earned_value(
        acts, today, proj["hours_per_day"], proj["base_rate_inr"],
        proj["labour_burden"], proj["inefficiency"], pct_by_name
    )
    ...
    # Build acts for the filtered df_view
    f_acts = []
